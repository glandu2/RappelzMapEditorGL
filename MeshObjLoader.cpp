...
namespace NS_OBJLOADER
{
	...
	*/
	void LoadTextures (unordered_map<string, MATERIAL>& Materials, TEXTURE_LOADER Loader, void* pPassThrough)
	{
		unordered_map<string, MATERIAL>::iterator it, itend;
	...
	*/
	void LoadMTLFile (const char *szFile, unordered_map<string, MATERIAL>& Materials, const string& sMaterialBasePath)
	{
...
*/
	...
	bool LoadOBJFile (const char *szFile, MESH& Mesh, unordered_map<string, MATERIAL>& Materials, const char* szMaterialBasePath, bool bSortByMaterial, bool bIgnoreMaterials)
	{
		setlocale(LC_ALL, "C");	//avoid bug using float with scanf (caused by different decimal point)
		...
		while (fscanf (pFile, "%s", szID) > 0)
		{
			...
			if (strcmp (szID, "vn") == 0)	// line declares a normal
			{
				...
				glm::vec3 v;
				fscanf (pFile, "%f %f %f", &v.x, &v.y, &v.z);
				glm::normalize(v);	// make sure normals are indeed normalized
			...
			if (strcmp (szID, "f") == 0)	// line declares a face
			{
				...
				if (Face.m_Vertices.size () >= 3)
				{
					...
					Face.m_vNormal = glm::normalize(glm::cross(v3 - v2, v1 - v2));
...
*/
	...
	void FACE::ComputeTangentSpaceVectors (const MESH& Mesh)
	{
		...
		const glm::vec3 p1 = Mesh.m_Positions[m_Vertices[0].m_uiPositionID];
		const glm::vec3 p2 = Mesh.m_Positions[m_Vertices[1].m_uiPositionID];
		const glm::vec3 p3 = Mesh.m_Positions[m_Vertices[2].m_uiPositionID];
		...
		m_vTangent = T;
		m_vBiTangent = glm::normalize(glm::cross(m_vNormal, m_vTangent));
	}
}
