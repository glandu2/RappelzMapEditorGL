...
namespace NS_OBJLOADER
{
	...
	*/
	struct MESH
	{
		deque<glm::vec3> m_Positions;
		deque<glm::vec3> m_Normals;
		deque<TEXCOORD> m_TexCoords;
		deque<FACE> m_Faces;
	...
	/*! Can handle OBJ-files that contain vertices ("v"), normals ("vn") and tex-coords ("vt"). Any of vt and vn can be missing.
		...
		MESH MyMesh;
		unordered_map<string, MATERIAL> MyMaterials;
		if (LoadOBJFile ("MyApplications\\Meshes\\SomeMesh.obj", MyMesh, MyMaterials, "MyApplication\\Textures\\"))
		{
			...
		}
	*/
	bool LoadOBJFile (const char *szFile, MESH& Mesh, unordered_map<string, MATERIAL>& Materials, const char* szMaterialBasePath = "", bool bSortByMaterial = true, bool bIgnoreMaterials = false);
